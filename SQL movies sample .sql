...
select genre , MIN('release_year') from Movie group by genre; -- TBR
...
select Director_name,count('movie_name') as 'current_count',no_of_movies as'total_count' from Movie,Director WHERE Director.Director_ID=Movie.director_id group by Movie.Director_ID,Director_name,no_of_movies;

;
--Q6. Display the name of lead_1 actors, their count of movies and their total DVD sales.
select lead_1, count('dvd.moviename'), sum('sale') from movie,dvd where movie.movie_name=dvd.movie_name group by movie.lead_1,dvd.Movie_name;
